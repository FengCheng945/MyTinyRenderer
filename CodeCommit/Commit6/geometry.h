...
{
	assert(NROW == NCOL && NROW == 3);

	Matrix<T, NROW, NCOL> adj;
	adj.m[0][0] = +(m[1][1] * m[2][2] - m[2][1] * m[1][2]);
	adj.m[0][1] = -(m[1][0] * m[2][2] - m[2][0] * m[1][2]);
	adj.m[0][2] = +(m[1][0] * m[2][1] - m[2][0] * m[1][1]);
	adj.m[1][0] = -(m[0][1] * m[2][2] - m[2][1] * m[0][2]);
	adj.m[1][1] = +(m[0][0] * m[2][2] - m[2][0] * m[0][2]);
	adj.m[1][2] = -(m[0][0] * m[2][1] - m[2][0] * m[0][1]);
	adj.m[2][0] = +(m[0][1] * m[1][2] - m[1][1] * m[0][2]);
	adj.m[2][1] = -(m[0][0] * m[1][2] - m[1][0] * m[0][2]);
	adj.m[2][2] = +(m[0][0] * m[1][1] - m[1][0] * m[0][1]);
	return adj;
}

//template<typename T, size_t NROW, size_t NCOL>
//void Matrix<T, NROW, NCOL>::inverse()
//{
//	assert(NROW == NCOL);
//	Matrix<T, NROW, NCOL>M(m);
//	Matrix<T, NROW, NCOL>E; E.identity();
//
//	for (int i = 0; i < NROW; i++)
//	{
//		for (int j = 0; j < NCOL; j++)
